...
Y_sg_ = double(subs(Y_sg,'s',1i*(W0+dW)));
Y_sg = double(subs(Y_sg,'s',1i*W0));
    
if 0

% Prepare star-delta conversion by adding new buses
% Notes: Y_sg has to be same for all SGs, i.e., R and L should be same.
...
for i = Ibuse_1st:N_Bus
    Hinv(i,i) = 1/(kp_pll{i}/kp_pll{n_i_ref});
    Hinv(i,i) = double(Hinv(i,i));
end
...
%     [theta]   [1 ,0] [theta ]   [0 ]
Ai = [0,0;ki_pll{n_i_ref},0];
Bi = [1;kp_pll{n_i_ref}];
Ci = [ki_pll{n_i_ref},0;1,0];
Di = [kp_pll{n_i_ref};0];
...
% K_ = diag([1,1,1,1,1]);
% Gamma_ = K_/1000;
% Hinv_ = diag([0.1,1,10,100,1000]);
% T1cl = feedback(Tss*Hinv_,K_,feedin,feedout_L1);
% T12cl = feedback(T1cl,Gamma_,feedin,feedout_L2);
...
if isempty(find(real(pole_sys_T12cl)>1e-9, 1))
    fprintf('Stable!\n')
else
    fprintf('Unstable!\n')
end